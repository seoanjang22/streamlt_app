...
st.sidebar.caption("💡 실제 미세한 처짐을 구조별로 뚜렷하게 비교하기 위한 배율입니다.")
...
st.markdown("하드보드지 구조물의 단면 모양과 하중에 따른 처짐 현상을 3D로 입체적으로 관찰하는 실험실입니다.")
...
with st.container():
    ...
    with c1:
        st.button("🔄 왼쪽으로 30° 회전", on_click=rotate_view, args=(-30,), use_container_width=True)
    with c2:
        st.button("⬇️ 정면 보기", on_click=reset_angle, use_container_width=True)
    with c3:
        st.button("🔄 오른쪽으로 30° 회전", on_click=rotate_view, args=(30,), use_container_width=True)
    ...
    for i in range(5 - st.session_state.w500):
        draw_cylinder(ax, -300 + (i*35), 10, -25, 12, 25, '#eab308')
    for i in range(5 - st.session_state.w100):
        draw_cylinder(ax, -290 + (i*35), -15, -25, 8, 12, '#cbd5e1')
    ...
    if P_newton > 0:
        ax.plot([center_x, center_x], [0, 0], [center_z, string_bottom_z], color='#64748b', linewidth=1.5)
...
# --- [6] 결과 수치 출력 (Metric 디자인 개선) ---
st.subheader("📊 실시간 수치 해석 결과")
col_res1, col_res2, col_res3, col_res4 = st.columns(4)

with col_res1:
    st.metric(label="⚖️ 총 하중 (Mass)", value=f"{total_mass_kg:.1f} kg")
with col_res2:
